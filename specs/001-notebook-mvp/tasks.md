...
- [x] T041 [US3] CHECKPOINT: execute quickstart.md §US3 scenarios 1–7 manually; SC-003 timing spot-check → story done
...
- [x] T042 [P] [US4] Interpolation lib in `src/lib/interpolate.ts`: `interpolate(text, vars)` (single pass, no recursion), `resolveRequest(request, vars)` covering url/header names/header values/body, `collectUnresolved(request, vars)` returning placeholder names; exhaustive tests in `src/lib/interpolate.test.ts` (FR-016/FR-017, adjacent tokens, `{{` literals, empty values)
- [x] T043 [P] [US4] Env var actions in `src/state/notebookStore.ts`: `addEnvVar`, `updateEnvVar`, `deleteEnvVar` with name regex `^[A-Za-z_][A-Za-z0-9_]*$` + uniqueness validation (FR-014); tests: duplicate rejection, deletion allowed while referenced (edge case)
- [x] T044 [US4] Env panel UI in `src/components/env/EnvPanel.tsx`: table CRUD, secret toggle, masked display `••••` with per-row reveal action (FR-015), inline validation errors; wire into `NotebookShell` as collapsible `Panel`; Tailwind semantic utilities + `Button` primitives
- [x] T045 [US4] Wire interpolation into HTTP run in `src/state/notebookStore.ts` `startHttpRun`: `collectUnresolved` → if any, block and surface warning dialog naming placeholders (FR-017); else send `resolveRequest` output to `runHttp`; unit test both paths
- [x] T070 [US4] Env var highlighting + autocomplete polish (inserted 2026-08-04): `src/lib/interpolate.ts` `tokenizePlaceholders` + `src/lib/envAutocomplete.ts` (`findActivePlaceholder`, `getEnvSuggestions`, `applyEnvSuggestion`) with unit tests; `src/components/env/InterpolatedTextPreview.tsx` (resolved=info / missing=warning chips, tooltips `name = value`, `name = •••••••• Secret`, `name is not defined` — secret values never rendered) + `src/components/env/EnvAwareInput.tsx` (combobox/listbox autocomplete after unclosed `{{`, case-insensitive prefix-then-contains filtering, ArrowUp/Down/Enter/Tab/Escape + click, fragment-only insertion, "No matching variables" empty state) with component tests; wired into `HttpCell` URL + header values (previews incl. body). Body in-editor autocomplete via CodeMirror deferred as follow-up; PHP cells excluded; no auto-run, no variable creation from suggestions
